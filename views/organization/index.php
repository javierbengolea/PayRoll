...
<div class="page-header">
    <div>
        <h1>Organización</h1>
        <div class="subtitle">Departamentos y puestos. Los sueldos básicos se definen en <a href="<?= url('categories') ?>">Categorías</a>.</div>
    </div>
</div>

<ul class="nav nav-tabs mb-3">
    <li class="nav-item"><a class="nav-link<?= $tab === 'departments' ? ' active' : '' ?>" href="<?= url('organization') ?>">Departamentos</a></li>
    <li class="nav-item"><a class="nav-link<?= $tab === 'positions' ? ' active' : '' ?>" href="<?= url('organization', ['tab' => 'positions']) ?>">Puestos</a></li>
</ul>
...
<div class="card">
    ...
    <div class="table-responsive">
        <table class="table align-middle mb-0">
            <thead><tr><th>Puesto</th><th>Departamento</th><th class="num">Empleados</th><th>Estado</th><th></th></tr></thead>
            <tbody>
                    ...
                    <td class="text-end text-nowrap">
                        <?php if (can('rrhh')): ?>
                            <button class="btn btn-sm btn-outline-secondary" data-bs-toggle="modal" data-bs-target="#posModal" data-title="Editar puesto"
                                    data-fill='<?= e(json_encode(['id' => $p['id'], 'name' => $p['name'], 'department_id' => $p['department_id'], 'active' => $p['active']])) ?>'><i class="bi bi-pencil"></i></button>
